...
from connect4.utils import Integer, get_diagonals_primary, get_diagonals_secondary, get_pts, get_valid_actions
import time
import functools
...
class AIPlayer:
    ...
    def expectimax(self, state, player, depth):
        ...
        if curr>=self.time - 0.5:
            self.terminate = True
            return get_pts(self.player_number,board)-get_pts(2 if self.player_number == 1 else 1,board)
        elif depth==0:
            return get_pts(self.player_number,board)-get_pts(2 if self.player_number == 1 else 1,board)
        else:
